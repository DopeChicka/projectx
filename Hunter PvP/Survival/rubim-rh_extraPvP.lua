...
local function GeneralPvP()
    --Interrupts Healing
    if Target:IsInterruptibleHeal() and interruptSpell():IsReady(interruptSpell()) and Target:HealthPercentage() <= 80 then
        return interruptSpell():Cast()
    end

    --Interrupts CC
    if Target:IsInterruptibleCC() and interruptSpell():IsReady(interruptSpell()) and Target:HealthPercentage() <= 95 then
        return interruptSpell():Cast()
    end
...
local function PvPRotation(specID)
    if specID == Survival then
        ...
        if Target:IsAPlayer() or Target:IsAPvPDummy() or select(2, IsInInstance()) == "arena" or select(2, IsInInstance()) == "battleground" then
            if RubimRH.CDsON() then
                ...
                if HRSurv.MongooseBite:IsReady() and (Player:BuffStack(HRSurv.MongooseFuryBuff) == 5 and Player:Focus() > 60) then
    ...
    SurvRotation = function()
        ...
        return 236329
            ...
            DeathCC = function()
                if not Arena.arena1:IsImmune() and Arena.arena1:CastingCC() and Arena.arena1:IsTargeting(Player) and HRSurv.FeignDeath:IsCastable() and Target:CastPercentage() >= randomGenerator("Reflect") then
                    return HRSurv.FeignDeath:Cast()
                end

                if not Arena.arena2:IsImmune() and Arena.arena2:CastingCC() and Arena.arena2:IsTargeting(Player) and HRSurv.FeignDeath:IsCastable() and Target:CastPercentage() >= randomGenerator("Reflect") then
                    return HRSurv.FeignDeath:Cast()
                end

                if not Arena.arena3:IsImmune() and Arena.arena3:CastingCC() and Arena.arena3:IsTargeting(Player) and HRSurv.FeignDeath:IsCastable() and Target:CastPercentage() >= randomGenerator("Reflect") then
                    return HRSurv.FeignDeath:Cast()
                end

                if not Target:IsImmune() and Target:CastingCC() and Target:IsTargeting(Player) and HRSurv.FeignDeath:IsCastable() and Target:CastPercentage() >= randomGenerator("Reflect") then
                    return HRSurv.FeignDeath:Cast()
                end
            end
            ...
            StingSpell = function()
                ...
                if not Arena.arena1:IsImmune() and HRSurv.ScorpidSting:IsCastable() and Arena.arena1:IsBursting() and Arena.arena1:IsDisarmable() and Arena.arena1:MinDistanceToPlayer(true) <= 40 then
                    RubimRH.Arena1Icon(HRSurv.ScorpidSting:Cast())
                    return
                end

                if not Arena.arena2:IsImmune() and HRSurv.ScorpidSting:IsCastable() and Arena.arena2:IsBursting() and Arena.arena2:IsDisarmable() and Arena.arena2:MinDistanceToPlayer(true) <= 40 then
                    RubimRH.Arena2Icon(HRSurv.ScorpidSting:Cast())
                    return
                end

                if not Arena.arena3:IsImmune() and HRSurv.ScorpidSting:IsCastable() and Arena.arena3:IsBursting() and Arena.arena3:IsDisarmable() and Arena.arena3:MinDistanceToPlayer(true) <= 40 then
                    RubimRH.Arena3Icon(HRSurv.ScorpidSting:Cast())
                    return
                end
            end
			
	SurvArena = function()
			if not Arena.arena1:IsImmune() and Arena.arena1:CastingHealing() and Arena.arena1:IsInterruptible() and HRSurv.Muzzle:IsReady() and Target:HealthPercentage() <= 40 then
            RubimRH.arena1.texture:SetColorTexture(0, 0.56, 0, 1)
            return
			end

			if not Arena.arena2:IsImmune() and Arena.arena2:CastingHealing() and Arena.arena2:IsInterruptible() and HRSurv.Muzzle:IsReady() and Target:HealthPercentage() <= 40 then
            RubimRH.arena2.texture:SetColorTexture(0, 0.56, 0, 1)
            return
			end

			if not Arena.arena3:IsImmune() and Arena.arena3:CastingHealing() and Arena.arena3:IsInterruptible() and HRSurv.Muzzle:IsReady() and Target:HealthPercentage() <= 40 then
            RubimRH.arena3.texture:SetColorTexture(0, 0.56, 0, 1)
            return
			end

			if not Arena.arena1:IsImmune() and Arena.arena1:CastingCC() and Arena.arena1:IsInterruptible() and HRSurv.Muzzle:IsReady() then
            RubimRH.arena1.texture:SetColorTexture(0, 0.56, 0, 1)
            return
			end

			if not Arena.arena2:IsImmune() and Arena.arena2:CastingCC() and Arena.arena2:IsInterruptible() and HRSurv.Muzzle:IsReady() then
            RubimRH.arena2.texture:SetColorTexture(0, 0.56, 0, 1)
            return
			end

			if not Arena.arena3:IsImmune() and Arena.arena3:CastingCC() and Arena.arena3:IsInterruptible() and HRSurv.Muzzle:IsReady() then
            RubimRH.arena3.texture:SetColorTexture(0, 0.56, 0, 1)
            return
			end
	end
			
		SurvPvPBurst = function()
			--1. Serpent Sting (If serpent sting debuff is not on the target or has <3s left)
			if HRSurv.SerpentSting:IsReady() and (Player:BuffRemains(HRSurv.VipersVenomBuff) <= 3) then
            return HRSurv.SerpentSting:Cast()
			end
			...
			--5. Mongoose Bite (IF 5 STACKS and above 60 focus)
			if HRSurv.MongooseBite:IsReady() and (Player:BuffP(S.MongooseFuryBuff) or Player:Focus() > 60) then
            return HRSurv.MongooseBite:Cast()
			end
            ...
            if SurvArena() ~= nil then
                return SurvArena()
            end
			
			if SurvPvPBurst() ~= nil then
                return SurvPvPBurst()
            end

        if Target:Exists() then
            if not Target:IsImmune() and Target:CastingCC() and Target:IsTargeting(Player) and HRSurv.SpellFeignDeath:IsCastable() and Target:CastPercentage() >= randomGenerator("Reflect") then
                return HRSurv.SpellFeignDeath:Cast()
            end

            if not Target:IsImmune() and Target:IsBursting() and Target:IsDisarmable() and HRSurv.ScorpidSting:IsReady("Melee") then
                return HRSurv.ScorpidSting:Cast()
            end
        end
    end
end
